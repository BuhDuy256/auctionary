// src/mails/auction-ended-winner.template.ts

import { themes } from "../configs/themes.config";

interface AuctionEndedWinnerTemplateProps {
  sellerName: string;
  productName: string;
  productImage?: string;
  finalBidAmount: number;
  winnerName: string; // Masked winner name (e.g., "****Khoa")
  productUrl: string;
}

export const getAuctionEndedWinnerTemplate = ({
  sellerName,
  productName,
  productImage,
  finalBidAmount,
  winnerName,
  productUrl,
}: AuctionEndedWinnerTemplateProps): string => {
...
<head>
  ...
  <title>Auction Ended - Winner Confirmed</title>
</head>
  ...
  <table role="presentation" width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: ${
          ...
          <tr>
            <td style="padding: 0 40px;">
              ...
              <p style="color: ${
                ...
                Great news! Your auction for <strong style="color: ${
                  colors.textMain
                };">${productName}</strong> has ended with a winning bid. Please proceed to complete the order.
              </p>
                    ...
                    <table role="presentation" width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td style="padding-bottom: 10px;">
                          <span style="color: ${
                            colors.textMuted
                          }; font-size: 14px;">Final Winning Bid:</span>
                        </td>
                      </tr>
                      <tr>
                      ...
                      <tr>
                        <td>
                          <span style="color: ${
                            colors.textMain
                          }; font-size: 16px; font-weight: 600;">${winnerName}</span>
                        </td>
                      </tr>
                    </table>
                  </td>
    ...
    colors.statusSuccess
                    ...
                    <p style="color: ${
                      ...
                      The winner will complete the payment process. Once confirmed, you'll receive further instructions for order fulfillment.
                    </p>
                  </td>
                </tr>
              </table>
