...
export const validateRecaptchaV2 = async (
  ...
  try {
    ...
    if (!recaptchaToken) {
      return next(new BadRequestError("Please complete the reCAPTCHA verification"));
    }

    const result = await verifyRecaptchaV2(recaptchaToken);

    if (!result.success) {
      return next(new BadRequestError(result.message || "reCAPTCHA verification failed. Please try again."));
    }
